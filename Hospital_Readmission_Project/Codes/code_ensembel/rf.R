...
model11 = train(subset(model1,select=-c(target)),model1$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model22 = train(subset(model2,select=-c(target)),model2$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model33 = train(subset(model3,select=-c(target)),model3$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model44 = train(subset(model4,select=-c(target)),model4$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model55 = train(subset(model5,select=-c(target)),model5$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model66 = train(subset(model6,select=-c(target)),model6$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
model77 = train(subset(model7,select=-c(target)),model7$target,'rf',trControl=trainControl(method='cv',number=2,verboseIter = T))
...
final_no$NO=ifelse(final_no$NO>0.4,1,0)
final_no$Within30days=ifelse(final_no$Within30days>0.6,1,0)
...
# join in dataframe to make the final submission
final_cluster=data.frame(test_patientid,final_no$pred)
table(final_cluster$final_no.pred)
write.csv(final_cluster,"final_cluster_rf.csv")
